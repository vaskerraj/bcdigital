...
import { message, Table, Pagination, Select, Tag, Input, AutoComplete, Button, Tooltip } from 'antd';
const { Option } = Select;
...
const UnapprovedProducts = ({ productData, total }) => {
    ...
    const columns = [
        ...
        {
            title: 'Unapproved At',
            dataIndex: 'unApprovedAt',
            key: 'unApprovedAt',
            render: (text, record) => <>{moment(record.products[0].approved.approvedAt).format("DD MMM YYYY")}</>,
        },
        {
            title: 'Action',
            key: 'action',
    ...
    return (
        <Wrapper onActive="unapprovedProduct" breadcrumb={["Products", "UnApproved Products"]}>
            <div className="mb-5">
