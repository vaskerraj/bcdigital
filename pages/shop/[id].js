...
const SellerShopPage = ({ categoryAndBrand, total, products, maxPrice, categoryQuery, priceQuery, brandQuery }) => {
    ...
    const fetchSellerData = async () => {
        const { data } = await axiosApi.get("api/shop/" + sellerId);
        console.log(data);
        setSeller(data);
        const { data: bannerData } = await axiosApi.get("api/shop/banner/" + sellerId);
    ...
    return (
        <Wrapper mobileTabBar={mobileTabBarStatus}>
            <Head>
                <title>{seller.userId?.name}-Buy Online Product At Best Price In Nepal | BC Digital</title>
                ...
                <div className="col mt-4">
                    {seller !== '' &&
                        <div className="d-flex bg-white">
                            {seller.userId?.picture !== undefined &&
                                <div className="d-inline p-3">
                                    <Image src={`/uploads/sellers/${seller.userId?.picture}`} height="65" width="65" />
                                </div>
                            }
                            <div className="d-inline mt-3 font16" style={{ fontWeight: 500 }}>
                                {seller.userId?.name}
                            </div>
                        </div>
                    }
...
export async function getServerSideProps(context) {
    try {
        ...
        const { data: results } = await axios.post(`${process.env.api}/api/shop`, {
            id,
            category,
            price,
            brand,
